const REGEX_A = new RegExp("[àáâãä]",'g');
const REGEX_E = new RegExp("[èéêẽë]",'g');
const REGEX_I = new RegExp("[ìíîĩï]",'g');
const REGEX_O = new RegExp("[òóôõö]",'g');
    ...
    static preSanitize = function(text){
        if(text != undefined){
            return text.toLowerCase()
                .replace(new RegExp("[^A-zÀ-ú]",'g'),"") // remove symbols
                .replace(new RegExp("\\s", 'g'),""); // remove emmpy spaces
        }
    }

    static sanitize = function (text){                        
        let r=""
        if(text != undefined){            
            r = Utils.preSanitize(text);
            r = r.replace(REGEX_A,"a");                    
            r = r.replace(REGEX_E,"e");
            r = r.replace(REGEX_I,"i");                                   
            r = r.replace(REGEX_O,"o");            
            r = r.replace(REGEX_U,"u");
            r = r.replace("ç","c");
        }
        return r;                
    }
